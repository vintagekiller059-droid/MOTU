...
// React.memo prevents rerenders from parent or unrelated state changes
// AICore only rerenders when currentMode changes (via useUIStore)
export const AICore: React.FC = memo(() => {
  ...
  return (
    <div className="relative flex items-center justify-center w-[600px] h-[600px] select-none will-change-transform">
      ...
      {/* Outer Labeled HUD Rings */}
      <OrbitalRing radius={550} speed={50} title="PLANNING & CONTEXT" subtitle="Planning Layer" currentState={currentMode} />
      <OrbitalRing radius={450} speed={40} title="LEARNING" subtitle="Vision Processing" currentState={currentMode} reverse={true} />
      <OrbitalRing radius={370} speed={32} title="MEMORY" subtitle="Context Buffer" currentState={currentMode} />
      <OrbitalRing radius={290} speed={25} title="KNOWLEDGE" subtitle="Knowledge Base" currentState={currentMode} reverse={true} />
      <OrbitalRing radius={210} speed={18} title="COGNITIVE LAYERS" subtitle="Reasoning" currentState={currentMode} />

      {/* AI Heart — Stable, never moves */}
