...
class UserProfileModel(Base):
    """Stores the user's personal identity and preferences."""
    ...
    projects: Mapped[Optional[list]] = mapped_column(JSON, nullable=True, default=list)
    interests: Mapped[Optional[list]] = mapped_column(JSON, nullable=True, default=list)
    goals: Mapped[Optional[list]] = mapped_column(JSON, nullable=True, default=list)
    additional: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True, default=dict)

    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now, nullable=False)

    __table_args__ = (
        Index("idx_user_profiles_id", "id"),
    )